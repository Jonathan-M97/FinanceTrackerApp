...
fun AddTransactionScreenContent(
    ...
    Scaffold(
        ...
        Column(
                ...
                ExposedDropdownMenu(
                    ...
                    typeOptions.forEach { option ->
                        DropdownMenuItem(
                            text = { Text(option) },
                            onClick = {
                                editableItem.value = editableItem.value.copy(type = option)
            ...
            if (editableItem.value.type == "Expense") {
                ExposedDropdownMenuBox(
                    expanded = isBudgetDropdownExpanded.value,
                    onExpandedChange = { isBudgetDropdownExpanded.value = !isBudgetDropdownExpanded.value }
                ) {
                    OutlinedTextField(
                        value = editableItem.value.budgetName,
                        onValueChange = {}, // Keep empty to prevent manual typing
                        readOnly = true, // Make it non-editable
                        label = { Text("Budget") },
                        trailingIcon = {
                            ExposedDropdownMenuDefaults.TrailingIcon(expanded = isBudgetDropdownExpanded.value)
                        },
                        modifier = Modifier.menuAnchor() // Important for accessibility
                    )
                    ExposedDropdownMenu(
                        expanded = isBudgetDropdownExpanded.value,
                        onDismissRequest = { isBudgetDropdownExpanded.value = false }
                    ) {
                        budgetList.forEach { budget ->
                            DropdownMenuItem(
                                text = { Text(budget.name) },
                                onClick = {
                                    budget.id?.let { editableItem.value = editableItem.value.copy(budgetId = it, budgetName = budget.name) } // id is possible null be prepared
                                    isBudgetDropdownExpanded.value = false
                                }
                            )
                        }
                    }
